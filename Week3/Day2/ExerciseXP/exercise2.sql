...
-- Lets get all the dates that accounts were created. Write a query to select all the create_date from 
...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title = 'Runaway Bride'
-- Write a query which should check if your favorite movie exists in the database. Have your query 
-- get the film ID, title, description, length and the rental rate, these details can be found in 
-- the “film” table.

SELECT film_id, title, description, length, rental_rate FROM film WHERE title ILIKE 'Ru%'
-- No luck finding your movie? Maybe you made a mistake spelling the name. Write a query to get the 
-- film ID, title, description, length and the rental rate of all the movies starting with the two 
-- first letters of your favorite movie.
...
SELECT film.film_id, film.title, inventory.film_id, inventory.store_id
FROM film
...
-- Bonus You want to be able to see how your sellers have been doing? Write a query to get the 
-- customer’s id, names (first and last), the amount and the date of payment ordered by the id of the 
